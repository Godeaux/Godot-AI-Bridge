...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- game_events — drain buffered events that happened between snapshots "
        "(collisions, animations finishing, node lifecycle, property changes, "
        "scene transitions). Snapshots include pending_events count — when > 0, "
        "call game_events() to see what you missed.\n"
        "- game_add_watch / game_remove_watch — monitor specific properties "
        "(health, score, ammo) for changes; changes appear in game_events()\n\n"
